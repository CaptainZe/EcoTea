...
- [x] 域名 ICP 备案已通过  
- [x] 云解析 A 记录已指向本机公网 IP  
- [x] 安全组入站已放行 **80**、**443**  
- [x] admin（8080）、api（8081）进程已在本机监听  
- [x] 两张 SSL 证书已签发（`api.ecotea.cn`、`admin.ecotea.cn`）
...
```bash
# 应解析到本机公网 IP（示例：8.135.45.189）
ping -c 3 api.ecotea.cn
ping -c 3 admin.ecotea.cn
```

自动 DNS 验证签发证书时，解析中可能出现 `_dnsauth.api` / `_dnsauth.admin` 的 TXT 记录；**证书已签发后可删可留**，不影响网站访问。
...
- 每个业务域名各一张：`api.ecotea.cn`、`admin.ecotea.cn`（绑定后通常不可改域名）  
- 验证方式：当前产品多为 **自动 DNS 验证**（需域名解析在同一账号云解析下）  
- CSR：**系统生成**；密钥算法：`RSA_2048`  
- 签发后：证书列表 → **下载** → 服务器类型选 **Nginx**

说明：个人测试类证书偏测试用途、无 SLA；生产长期也可日后改 Let’s Encrypt 自动续期，Nginx 反代配置可复用，仅换证书路径。
...
```bash
# 创建证书目录
mkdir -p /etc/nginx/ssl/api.ecotea.cn /etc/nginx/ssl/admin.ecotea.cn
...
4. 菜单 / H5 链接一律使用 `https://api.ecotea.cn/...`
...
| 证书目录 | `/etc/nginx/ssl/{api,admin}.ecotea.cn/` |
...
| 证书名称不匹配               | api / admin 的 pem、key 装反             |
